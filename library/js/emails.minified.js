...
{data=$.parseJSON(data);$("input#email_sender").val(data.sender);$("input#email_subject").val(data.subject);$("textarea#email_content").html(data.content)})}})});function sendEmail()
...
if(valid==!0)
{setTimeout(function()
{$.post("/library/php/posts/mailserver/send.php",{customerID:$("div.tab.js-load-emails").attr("e-mail-customer-id"),orderID:$("input#email_orderID").val(),sender:$("input#email_sender").val(),receiver:$("input#email_receiver").val(),subject:$("input#email_subject").val(),content:$("textarea#email_content").val()}).done(function(data)
{loadEmails($("input#email_customerID").val())})},1000);setTimeout(function()
{$("input#email_sender, input#email_subject, textarea#email_content, select#email_template").val("");$("input#send_email").val($("input#send_email").attr("original")).removeClass("no-action");$('div.content').animate({scrollTop:200},1000)},1500)}}
