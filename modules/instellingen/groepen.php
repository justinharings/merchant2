<?php
$mb->_runFunction("authorization", "userPermission", array($_SESSION['userID'], "SET_VG", 1));

$data = $mb->_runFunction("groups", "view", array($_SESSION['merchantID'], (isset($_GET['search_string']) ? trim($_GET['search_string'], "/") : ""), "groups.name", "0,50"));
...
<ul class="breadcrumbs">
	...
	<li><?= $mb->_translateReturn("menu", "manage-groups") ?></li>
</ul>
...
<div class="view-options">
	...
	<div class="button fa fa-question-circle" title="U kunt verschillende verkoopgroepen aanmaken die handig zijn voor uw statistieken en rapporten. Een product hangt u onder een verkoopgroep."></div>
</div>
...
<table class="view <?= $mb->num_rows($data) ? "hoverable" : "" ?>">
	...
	<tbody>
		...
		if($mb->num_rows($data))
		{
			...
			{
				?>
				<tr click="<?= "/" . _LANGUAGE_PACK . "/modules/" . $_GET['module'] . "/" . trim($_GET['file'], "/") . $form . $value['groupID'] ?>">
					<td><?= $value['name'] ?></td>
